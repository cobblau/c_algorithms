...
int main(int argc, char **argv){
	...
	struct cobb_list    *tmp;
	struct list_head    *pos, *q;
	unsigned int         i;

	struct cobb_list mylist;
	init_list_head(&mylist.list);

	/* adding elements to mylist */
	for (i = 5; i != 0; --i) {
		tmp = (struct cobb_list *) malloc(sizeof(struct cobb_list));
	...
	/* now you have a circularly linked list of items of type struct cobb_list.
	...
	list_for_each(pos, &mylist.list){
		 ...
		 * of type struct cobb_list. But we need to access the item itself not the 
		 ...
		 */
		 tmp = list_entry(pos, struct cobb_list, list);
		  ...
		  * struct cobb_list item it is embedded in!
		  */
	...
	/* now let's be good and free the cobb_list items. since we will be removing items
